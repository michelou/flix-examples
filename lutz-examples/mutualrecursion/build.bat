...
if "%__ARG:~0,1%"=="-" (
    ...
    ) else if "%__ARG%"=="-java" ( set _JAVA=1
...
if %_JAVA%==1 (
    ...
    set _MAIN_CLASS=MutualRecursion
    set _COMMANDS=!_COMMANDS:run=run_java!
    set _COMMANDS=!_COMMANDS:compile=compile_java!
) else if %_NIGHTLY%==1 (
    set __NIGHTLY_JAR=
...
if %_DEBUG%==1 (
    ...
    echo %_DEBUG_LABEL% Options    : _JAVA=%_JAVA% _NIGHTLY=%_NIGHTLY% _VERBOSE=%_VERBOSE% 1>&2
...
echo     %__BEG_O%-java%__END%       select Java example instead of Flix one
echo     %__BEG_O%-nightly%__END%    use lastest Flix nightly build if locally available
...
echo     %__BEG_O%run%__END%         execute the generated program "%_PROJECT_NAME%"
...
if exist "%_BUILD_DIR%\src\Main.flix" del /q "%_BUILD_DIR%\src\Main.flix"
...
set __SOURCE_FILES=
set __N=0
for /f "delims=" %%f in ('dir /b /s "%_SOURCE_MAIN_DIR%\*.java" 2^>NUL') do (
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_JAVAC_CMD%" -d "%_CLASSES_DIR%" %__SOURCE_FILES% 1>&2
) else if %_VERBOSE%==1 ( echo Compile %__N_FILES% to directory "!_CLASSES_DIR:%_ROOT_DIR%=!" 1>&2
)
call "%_JAVAC_CMD%" -d "%_CLASSES_DIR%" %__SOURCE_FILES%
if not %ERRORLEVEL%==0 (
...
:run_java
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_JAVA_CMD%" -cp "%_CLASSES_DIR%" %_MAIN_CLASS% 1>&2
) else if %_VERBOSE%==1 ( echo Execute Java program "%_MAIN_CLASS%" 1>&2
)
call "%_JAVA_CMD%" -cp "%_CLASSES_DIR%" %_MAIN_CLASS%
if not %ERRORLEVEL%==0 (
