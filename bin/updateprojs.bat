...
set _MAKE_FLIX_VERSION_OLD=FLIX_VERSION  = 0.44.0
set _MAKE_FLIX_VERSION_NEW=FLIX_VERSION  = 0.43.0
...
if exist "%__MAKEFILE_INC%" (
    if %_DEBUG%==1 echo %_DEBUG_LABEL% "%_GREP_CMD%" -q "%_MAKE_FLIX_VERSION_OLD%" "!__MAKEFILE_INC!" 1>&2
    call "%_GREP_CMD%" -q "%_MAKE_FLIX_VERSION_OLD%" "!__MAKEFILE_INC!"
    if !ERRORLEVEL!==0 (
        if %_DEBUG%==1 echo %_DEBUG_LABEL% "%_SED_CMD%" -i "s@%_MAKE_FLIX_VERSION_OLD%@%_MAKE_FLIX_VERSION_NEW%@g" "!__MAKEFILE_INC!" 1>&2
        call "%_SED_CMD%" -i "s@%_MAKE_FLIX_VERSION_OLD%@%_MAKE_FLIX_VERSION_NEW%@g" "!__MAKEFILE_INC!"
        call "%_UNIX2DOS_CMD%" -q "!__INPUT_FILE!"
        set /a __N_INC+=1
    )
) else (
