...
if defined _SEARCH_FLIX (
    if exist "%FLIX_HOME%\" (
        call :search "%FLIX_HOME%" 1
        if not !_EXITCODE!==0 goto end
    )
)
...
if "%__ARG:~0,1%"=="-" (
    ...
    ) else if "%__ARG%"=="-flix" ( set _SEARCH_FLIX=1
...
goto args_loop
:args_done
set __SOME=%_SEARCH_FLIX%%_SEARCH_IVY%%_SEARCH_MAVEN%%_SEARCH_JAVA%
...
if %_DEBUG%==1 (
    echo %_DEBUG_LABEL% Options   : _SEARCH_FLIX=%_SEARCH_FLIX% _SEARCH_IVY=%_SEARCH_IVY% _SEARCH_MAVEN=%_SEARCH_MAVEN% _SEARCH_JAVA=%_SEARCH_JAVA% 1>&2
...
echo     %__BEG_O%-flix%__END%         search in Flix library directories
